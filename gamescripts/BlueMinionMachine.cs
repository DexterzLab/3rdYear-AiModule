...
public class BlueMinionMachine : MonoBehaviour {
    ...
    private RedMinionMachine _target;
    private RedTower Turret_target;
    ...
    IEnumerator FollowPath()
    {
        ...
        while (true)
        {
            if (transform.position == currentWaypoint)
            {
                ...
                //if target has reached destination then stop following the path
    ...
    private Transform CheckForAggro()
    {
        ...
        if (minionDistance <= ViewDistance)
        {
            ...
            if (Physics.Raycast(pos, transform.forward, out hit, ViewDistance))
            {
                ...
                var minion = hit.collider.GetComponent<RedMinionMachine>();
